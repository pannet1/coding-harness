...
def get_orgs(user):
    output = run_command(f'gh api users/{user}/orgs --jq ".[].login"')
    return output.splitlines() if output else []

def get_repos(owner):
    output = run_command(f'gh repo list {owner} --limit 1000 --json name,primaryLanguage,isFork')
